...
export const helpPt = {
  ...
  "help.login.body":
    "O Portal Wellmix organiza a importação do pedido do cliente até a entrega. Cada participante (cliente, Wellmix, fornecedor, agência, despachante, companhia marítima, transportador, jurídico) entra com a própria conta e vê apenas o que lhe cabe.",
  "help.login.roles":
    "Cliente: cria solicitações, vê a proposta, paga o sinal, acompanha a timeline e confirma o recebimento.\nWellmix (admin e operador): abre RFQs, compara cotações, escolhe fornecedor, confirma pagamentos, designa parceiros e acompanha tudo na Control Tower.\nFornecedor: responde cotações, cumpre checklists, envia fotos e peso, confirma pagamentos e vê a conta corrente.\nAgência: aprova a arte da embalagem.\nDespachante, companhia marítima e transportador: registram cada um a sua etapa.\nJurídico: acompanha multas.",
  ...
  "help.requests.new.steps":
    "Quantidade e unidade são obrigatórias; o prazo desejado orienta a cotação.\nEspecificação: material, medidas, cores, embalagem, certificações.\nAnexos: fotos de referência, desenhos, planilhas.\nAo enviar, a Wellmix é avisada e abre a RFQ.",
  "help.request.body":
    "Acompanhe a solicitação por status. A Wellmix vê e faz tudo; o cliente vê a proposta, o sinal e o andamento, sem ver fornecedores nem custos internos.",
  "help.request.steps":
  ...
  "help.quote.body":
    "Esta é a sua cotação para a solicitação ao lado. Informe o preço por unidade, a moeda, o prazo de produção em dias e as condições (Incoterm, forma de pagamento). Você pode reenviar enquanto a Wellmix não decidir.",
  "help.quote.steps":
  ...
  "help.stage.INSPECTION":
    "Fornecedor envia foto sem embalagem, com embalagem, na balança e o peso medido. Divergência acima da tolerância bloqueia a etapa até a Wellmix revisar.",
  "help.stage.SHIPPING":
    "Companhia marítima (ou a Wellmix, se não houver uma designada) informa data de embarque, previsão de chegada, navio e o BL.",
  "help.stage.CUSTOMS":
  ...
  "help.stage.DELIVERED":
    "Cliente confirma o recebimento (a Wellmix pode confirmar por ele). Com a confirmação o pedido é encerrado.",
  "help.stage.CLOSED": "Pedido encerrado. Nada mais a fazer.",
  "help.account.body":
    "Conta corrente do fornecedor: valor FOB de cada pedido, pagamentos registrados pela Wellmix, câmbio e saldo. O fornecedor confirma cada recebimento; isso também conclui o requisito da etapa de pagamento.",
  "help.penalties.body":
    "Multas registradas pela Wellmix por atraso, avaria ou descumprimento, com responsável, valor e evidência. O jurídico acompanha e altera a situação: contestada, paga ou cancelada.",
  "help.parties.body":
  ...
  "help.finance.steps":
    "Recebimento do cliente: registre no pedido (card Financeiro) o sinal e as parcelas seguintes, com comprovante.\nPagamento ao fornecedor: registre na etapa de pagamento com valor, moeda e câmbio; o fornecedor confirma.\nMargem: venda menos custo convertido. Informe o câmbio nos pagamentos para a margem deixar de ser estimativa.\nA receber e A pagar em destaque indicam pendência de caixa.",
...
export const helpEn: Record<HelpKey, string> = {
  ...
  "help.login.body":
    "The Wellmix Portal organizes the import from the customer's request to delivery. Each participant (customer, Wellmix, supplier, agency, customs broker, shipping line, carrier, legal) signs in with their own account and sees only what concerns them.",
  "help.login.roles":
    "Customer: creates requests, sees the proposal, pays the down payment, follows the timeline and confirms receipt.\nWellmix (admin and operator): opens RFQs, compares quotations, picks suppliers, confirms payments, assigns partners and follows everything on the Control Tower.\nSupplier: answers quotations, completes checklists, sends photos and weight, confirms payments and sees the account statement.\nAgency: approves the packaging artwork.\nCustoms broker, shipping line and carrier: each registers their own stage.\nLegal: follows penalties.",
  ...
  "help.requests.new.steps":
    "Quantity and unit are mandatory; the desired deadline guides the quotation.\nSpecification: material, dimensions, colors, packaging, certifications.\nAttachments: reference photos, drawings, spreadsheets.\nOn submit, Wellmix is notified and opens the RFQ.",
  "help.request.body":
    "Follow the request by status. Wellmix sees and does everything; the customer sees the proposal, the down payment and progress, without suppliers or internal costs.",
  "help.request.steps":
  ...
  "help.quote.body":
    "This is your quotation for the request on the left. Enter unit price, currency, production lead time in days and terms (Incoterm, payment). You may resubmit until Wellmix decides.",
  "help.quote.steps":
  ...
  "help.stage.INSPECTION":
    "Supplier sends photos without packaging, with packaging, on the scale and the measured weight. Divergence above tolerance blocks the stage until Wellmix reviews.",
  "help.stage.SHIPPING":
    "Shipping line (or Wellmix if none is assigned) informs shipping date, ETA, vessel and the BL.",
  "help.stage.CUSTOMS":
  ...
  "help.stage.DELIVERED":
    "Customer confirms receipt (Wellmix may confirm on their behalf). The order is then closed.",
  "help.stage.CLOSED": "Order closed. Nothing left to do.",
  "help.account.body":
    "Supplier account statement: FOB value per order, payments registered by Wellmix, FX rate and balance. The supplier confirms each receipt; that also completes the payment stage requirement.",
  "help.penalties.body":
    "Penalties registered by Wellmix for delays, damage or non-compliance, with responsible party, amount and evidence. Legal follows and updates the status: disputed, paid or cancelled.",
  "help.parties.body":
  ...
  "help.finance.steps":
    "Customer receipt: register on the order (Finance card) the down payment and the following installments, with proof.\nSupplier payment: register on the payment stage with amount, currency and FX rate; the supplier confirms.\nMargin: sale minus converted cost. Enter the FX rate on payments so the margin stops being an estimate.\nHighlighted Receivable and Payable indicate pending cash.",
  "help.notifications.body":
...
export const helpZh: Record<HelpKey, string> = {
  ...
  "help.login.body":
    "Wellmix 门户负责从客户需求到交付的整个进口流程。每位参与方（客户、Wellmix、供应商、设计公司、报关行、船公司、承运商、法务）使用各自账号登录，只看到与自己相关的内容。",
  "help.login.roles":
    "客户：创建需求、查看方案、支付定金、跟踪时间线并确认收货。\nWellmix（管理员和操作员）：发起询价、对比报价、选定供应商、确认付款、指定合作方，并在控制塔跟踪一切。\n供应商：回复报价、完成清单、上传照片和重量、确认收款、查看往来账。\n设计公司：审核包装设计稿。\n报关行、船公司和承运商：各自登记自己的阶段。\n法务：跟进罚款。",
  ...
  "help.requests.new.steps":
    "数量和单位为必填；期望交期用于指导报价。\n规格：材质、尺寸、颜色、包装、认证。\n附件：参考照片、图纸、表格。\n提交后 Wellmix 会收到通知并发起询价。",
  "help.request.body":
    "按状态跟踪需求单。Wellmix 可查看并执行全部操作；客户可看到方案、定金和进度，但看不到供应商和内部成本。",
  "help.request.steps":
    "已提交：Wellmix 选择供应商并发起询价。\n询价中：供应商通过收到的链接回复。\n已收到报价：Wellmix 对比后选定供应商，确定客户价格和定金。\n等待定金：客户付款；Wellmix 确认（可附凭证）后创建订单。\n已创建订单：点击“查看订单”跟踪各阶段。",
  "help.quote.body":
    "这是您对左侧需求的报价。请填写单价、币种、生产交期（天）和条款（贸易术语、付款方式）。在 Wellmix 做出决定前可重新提交。",
  "help.quote.steps":
  ...
  "help.stage.INSPECTION":
    "供应商提交无包装照片、带包装照片、称重照片和实测重量。超出容差将阻塞阶段，直到 Wellmix 复核。",
  "help.stage.SHIPPING":
    "船公司（未指定时由 Wellmix）填写装运日期、预计到达、船名和提单。",
  "help.stage.CUSTOMS":
    "报关行登记流程编号、清关文件、预计和实际费用以及放行。",
  "help.stage.TRANSPORT": "承运商填写车牌、预计送达、状态并确认送达。",
  "help.stage.DELIVERED":
    "客户确认收货（Wellmix 可代为确认）。确认后订单结案。",
  "help.stage.CLOSED": "订单已结案，无需操作。",
  "help.account.body":
    "供应商往来账：每单 FOB 金额、Wellmix 登记的付款、汇率和余额。供应商确认每笔收款，同时完成付款阶段的要求。",
  "help.penalties.body":
    "Wellmix 因延误、损坏或违约登记的罚款，含责任方、金额和证据。法务跟进并更新状态：争议中、已支付或已取消。",
  "help.parties.body":
  ...
  "help.finance.steps":
    "客户收款：在订单的“财务”卡片登记定金及后续款项，并附凭证。\n供应商付款：在付款阶段登记金额、币种和汇率；供应商确认。\n毛利：销售额减去折算成本。为付款填写汇率后毛利不再是估算。\n高亮的应收和应付表示待处理的现金。",
  "help.notifications.body":
